function jumpClogitAdjMLE(Y,Z,baseAlt=maximum(Y),adj,beta)
	...
	@defNLExpr(likelihood, sum{ sum{ (Y[i]==j)*(sum{ (Z[i,k,j]-Z[i,k,baseAlt])*bz[k],k=1:K2} + (adj[i,j]-adj[i,baseAlt]) ) ,j=1:J} - log(sum { exp( sum{ (Z[i,k,j]-Z[i,k,baseAlt])*bz[k],k=1:K2} + adj[i,j]-adj[i,baseAlt] ) ,j=1:J} ) ,i=1:n});
