function simulation2()
	...
	N    = convert(Int64,1.95e3);
	T    = 15;
	time = kron(ones(N),[1:T]');
	nloc = 3;
	J    = 2*nloc;

	## generate the data
	# initial lagged choice and experience
	LY     = rand(DiscreteUniform(1,J),N);
	exper0 = zeros(N);
	...
	# beta = 0;
	eul = .577215664901532860606512090082
	...
	fv     = zeros(N,J,convert(Int64,maximum(vec(exper0)))+T+2,T+1);
	fvtemp = zeros(N,J)
	dem    = zeros(N)
	p1     = zeros(size(fv));
	p2     = zeros(size(fv));
	...
	for t=T:-1:1 # loop backwards through time
		for k=convert(Int64,maximum(vec(exper0)))+T+1:-1:1 # loop over all possible values of experience
	...
	for t=1:T
		for j=1:J
			for k=1:convert(Int64,maximum(vec(exper0)))+T+1
	...
	lnEarn = zeros(N,T);
	for t=1:T
		lnEarn[:,t] = bcons + bmed*(Choice[:,t].==2) + bbig*(Choice[:,t].==3) + bexp*exper[:,t] + bexp2*exper[:,t].^2 + .3*randn(N);
	end

	#estimate wage equation
	bhatw = linreg([ones(sum(Choice[:].<=nloc)) obsloc[Choice.<=nloc].==2 obsloc[Choice.<=nloc].==3 exper[Choice.<=nloc] (exper[Choice.<=nloc]).^2],vec(lnEarn[Choice.<=nloc]));
	# println(hcat(vcat(bcons,bmed,bbig,bexp,bexp2),bhatw))
	...
	# return beta
	return b_ans,bs,bm,Y,LY,FY,lnEarn,obsloc,exper0,exper,T,N,nloc,time,p1,p2,p3,p4,p5,p6,fv
