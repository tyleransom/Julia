function JuMPxtlogitMLEnoHet()
	...
	@defNLExpr(likelihood, sum{ log( prod { (exp( sum{ X[i,t,k]*(bx[k]),k=1:K1} + sum{ (Z[i,t,k,1]-Z[i,t,k,2])*bz[k],k=1:K2} )/(1+exp( sum{ X[i,t,k]*(bx[k]),k=1:K1} + sum{ (Z[i,t,k,1]-Z[i,t,k,2])*bz[k],k=1:K2})))^((Y[i,t]==1)) *
	(1/(1+exp( sum{ X[i,t,k]*(bx[k]),k=1:K1} + sum{ (Z[i,t,k,1]-Z[i,t,k,2])*bz[k],k=1:K2})))^((Y[i,t]==2))	,t=1:T}  ) ,i=1:N});
