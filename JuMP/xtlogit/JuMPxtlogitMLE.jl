function JuMPxtlogitMLE()
	...
	@defNLExpr(likelihood, sum{ log( sum{ prod { (exp( sum{ X[i,t,k]*(bx[k]),k=1:K1} + sum{ (Z[i,t,k,1]-Z[i,t,k,2])*bz[k],k=1:K2} + sig*grid[g] )/(1+exp( sum{ X[i,t,k]*(bx[k]),k=1:K1} + sum{ (Z[i,t,k,1]-Z[i,t,k,2])*bz[k],k=1:K2} + sig*grid[g] )))^((Y[i,t]==1)) *
	(1/(1+exp( sum{ X[i,t,k]*(bx[k]),k=1:K1} + sum{ (Z[i,t,k,1]-Z[i,t,k,2])*bz[k],k=1:K2} + sig*grid[g] )))^((Y[i,t]==2))	,t=1:T}  * gridDen[g] * quad_weight[g] ,g=1:G}) ,i=1:N});
	...
	return bxOpt,bzOpt,sigOpt,hOpt,seOpt
end