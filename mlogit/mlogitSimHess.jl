...
function applyRestrGradHess(restrMat,grad,hess)
...
# function clogitHess(b::Vector,restrMat::Matrix,Y::Vector,X::Matrix,Z::Matrix,baseAlt::Int64=J,W::Vector=ones(size(Y,1),1))
function clogitHess(b,restrMat,Y,X,Z,baseAlt=J,W=ones(size(Y,1),1))
	...
	if K2>0 && K1>0 # hybrid of multinomial and conditional logit
		...
		for j=setdiff(1:J,baseAlt)
			...
			for jj=setdiff(1:J,baseAlt)
				...
				hess[(k-1)*K1+1:k*K1,K1*(J-1)+1:K1*(J-1)+K2]        += (X.*(W.*P[:,j]*ones(1,K1)))'*((Z[:,:,jj]-Z[:,:,baseAlt]).*((j==jj)-(P[:,jj]*ones(1,K2))))
				hess[K1*(J-1)+1:K1*(J-1)+K2,(kk-1)*K1+1:kk*K1]      += ((Z[:,:,j]-Z[:,:,baseAlt]).*(W.*P[:,j]*ones(1,K2)))'*(X.*((j==jj)-(P[:,jj]*ones(1,K1))))
				hess[K1*(J-1)+1:K1*(J-1)+K2,K1*(J-1)+1:K1*(J-1)+K2] += ((Z[:,:,j]-Z[:,:,baseAlt]).*(W.*P[:,j]*ones(1,K2)))'*((Z[:,:,jj]-Z[:,:,baseAlt]).*((j==jj)-(P[:,jj]*ones(1,K2))))
				kk+=1
			end
			k+=1
		end
	...
	elseif K1==0 && K2>0 # traditional conditional logit
		...
		for j=setdiff(1:J,baseAlt)
			kk = 1
			for jj=setdiff(1:J,baseAlt)
				hess+=((Z[:,:,j]-Z[:,:,baseAlt]).*(W.*P[:,j]*ones(1,K2)))'*((Z[:,:,jj]-Z[:,:,baseAlt]).*((j==jj)-(P[:,jj]*ones(1,K2))))
				k+=1
			end
			kk+=1
		end
...
function datagen()
	...
	tic()
	l,g,h = clogitHess(startval,[],Y,X,Z,J,wgts)
	println("Time spent evaluating clogitHess: ", toc())
