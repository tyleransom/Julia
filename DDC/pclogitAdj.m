function P=pclogit(b,Y,X,Z,baseAlt,adj,beta)
%PCLOGITADJ returns fitted probabilities from a conditional logit model
%   P = PCLOGITADJ(B,Y,X,Z,BASEALT,ADJ,BETA) 
...
%     Created
%   August 20, 2014
%     Changed name of function to match file name; added error checks
%==========================================================================
...
if K2>0 && K1>0
	...
	for j=setdiff(1:J,baseAlt)
		temp = X*b((k-1)*K1+1:k*K1) + (Z(:,:,j)-Z(:,:,baseAlt))*b2 + beta*(adj(:,j)-adj(:,baseAlt));
		num(:,j)=exp(temp);
		dem=exp(temp)+dem;
		k = k+1;
	end
elseif K1>0 && K2==0
	...
	k = 1;
	for j=setdiff(1:J,baseAlt)
		temp=X*b((k-1)*K1+1:k*K1) + beta*(adj(:,j)-adj(:,baseAlt));
		num(:,j)=exp(temp);
		dem=exp(temp)+dem;
		k = k+1;
	end
