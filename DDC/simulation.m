...
for t=T:-1:1 % loop backwards through time
	for k=max(exper0)+T+1:-1:1 % loop over all possible values of experience
		for j=1:J % loop over previous decision
            dem = exp(0       +bw*(bcons        + bexp*(k-1) + bexp2*(k-1).^2)+a(1,j)+am(1,j)+fv(:,1,k+1,t+1))+...
                  exp(alpha(1)+bw*(bcons + bmed + bexp*(k-1) + bexp2*(k-1).^2)+a(2,j)+am(2,j)+fv(:,2,k+1,t+1))+...
                  exp(alpha(2)+bw*(bcons + bbig + bexp*(k-1) + bexp2*(k-1).^2)+a(3,j)+am(3,j)+fv(:,3,k+1,t+1))+...
                  exp(0       +bb                                             +a(4,j)+am(4,j)+fv(:,4,k  ,t+1))+...
                  exp(alpha(1)+bb                                             +a(5,j)+am(5,j)+fv(:,5,k  ,t+1))+...
                  exp(alpha(2)+bb                                             +a(6,j)+am(6,j)+fv(:,6,k  ,t+1));
            fv(:,j,k,t) = beta*(log(dem)-psi(1));
            p1(:,j,k,t) = exp(0       +bw*(bcons        + bexp*(k-1) + bexp2*(k-1).^2)+a(1,j)+am(1,j)+fv(:,1,k+1,t+1))./dem;
            p2(:,j,k,t) = exp(alpha(1)+bw*(bcons + bmed + bexp*(k-1) + bexp2*(k-1).^2)+a(2,j)+am(2,j)+fv(:,2,k+1,t+1))./dem;
            p3(:,j,k,t) = exp(alpha(2)+bw*(bcons + bbig + bexp*(k-1) + bexp2*(k-1).^2)+a(3,j)+am(3,j)+fv(:,3,k+1,t+1))./dem;
            p4(:,j,k,t) = exp(0       +bb                                             +a(4,j)+am(4,j)+fv(:,4,k  ,t+1))./dem;
            p5(:,j,k,t) = exp(alpha(1)+bb                                             +a(5,j)+am(5,j)+fv(:,5,k  ,t+1))./dem;
            p6(:,j,k,t) = exp(alpha(2)+bb                                             +a(6,j)+am(6,j)+fv(:,6,k  ,t+1))./dem;
